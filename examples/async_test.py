...
def test_async_mode():
    ...
    # monitor.add_handler('update', statistics_handler)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if final_status['async_enabled']:
            print("📊 最终统计:")
            print(f"   总提交任务: {final_status['tasks_submitted']}")
            print(f"   总完成任务: {final_status['tasks_completed']}")
            print(f"   总失败任务: {final_status['tasks_failed']}")
            print(f"   总超时任务: {final_status['tasks_timeout']}")
        print("杂鱼♡～异步处理让杂鱼主人的体验更好了吧～")
        print("🎉" * 20)
    finally:
        monitor.stop() 